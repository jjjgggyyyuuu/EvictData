...
def evictions():
    ...
    # Apply search filter
    if search_term:
        filtered_evictions = [e for e in filtered_evictions if 
                             (search_term.lower() in e.get('tenant_name', '').lower()) or
                             (search_term.lower() in e.get('property_address', '').lower()) or
                             (search_term.lower() in e.get('case_number', '').lower())]
    ...
    if date_from:
        try:
            ...
            filtered_evictions = [e for e in filtered_evictions if 
                                 datetime.strptime(e.get('filing_date', '1900-01-01'), '%Y-%m-%d') >= date_from]
        except ValueError:
            pass
    ...
    if date_to:
        try:
            ...
            filtered_evictions = [e for e in filtered_evictions if 
                                 datetime.strptime(e.get('filing_date', '2100-12-31'), '%Y-%m-%d') <= date_to]
        except ValueError:
            pass
...
def view_eviction(eviction_id):
    ...
    # Calculate days remaining until eviction if in writ status
    if eviction.get('status') == 'writ' and eviction.get('writ_date'):
        writ_date = datetime.strptime(eviction.get('writ_date'), '%Y-%m-%d')
        eviction_date = writ_date + timedelta(days=AVERAGE_TIMELINES['writ_to_eviction'])
        days_remaining = (eviction_date - datetime.now()).days
        eviction['days_remaining'] = max(0, days_remaining)
...
def update_eviction(eviction_id):
    ...
    for i, eviction in enumerate(EVICTIONS_DB):
        if eviction.get('id') == eviction_id:
            ...
            if judgment_amount:
                eviction['judgment_amount'] = float(judgment_amount)
            
            court_date = request.form.get('court_date')
            if court_date:
                eviction['court_date'] = court_date
            
            judgment_date = request.form.get('judgment_date')
            if judgment_date:
                eviction['judgment_date'] = judgment_date
            
            writ_date = request.form.get('writ_date')
            if writ_date:
                eviction['writ_date'] = writ_date
            ...
            if filename.endswith('.json'):
                ...
                try:
                    ...
                    report_files.append({
                        ...
                        'type': 'Eviction Report' if 'eviction_report' in filename else 'Import Report'
                    })
                except:
                    pass
...
def generate_eviction_report():
    ...
    report_title = request.form.get('title', 'Eviction Report')
    ...
    if report_format == 'markdown':
        ...
        with open(report_md_path, 'w') as f:
            ...
            f.write(f"- Date Range: {date_from if isinstance(date_from, str) else date_from.strftime('%Y-%m-%d') if date_from else 'Any'} to {date_to if isinstance(date_to, str) else date_to.strftime('%Y-%m-%d') if date_to else 'Any'}\n\n")
            ...
            # Financial summary
            total_judgment = sum(float(e.get('judgment_amount', 0) or 0) for e in filtered_evictions)
            total_back_rent = sum(float(e.get('back_rent_amount', 0) or 0) for e in filtered_evictions)
            ...
            for eviction in filtered_evictions:
                ...
                judgment_amount = f"${float(eviction.get('judgment_amount', 0)):.2f}" if eviction.get('judgment_amount') else 'N/A'
                
                f.write(f"| {eviction.get('id', 'Unknown')} | {tenant} | {property_addr} | {filing_date} | {status.capitalize()} | {judgment_amount} |\n")
            ...
            for eviction in filtered_evictions:
                ...
                f.write(f"### Case {eviction.get('case_number', 'Unknown')}\n\n")
                f.write(f"- **Tenant**: {tenant}\n")
                f.write(f"- **Property**: {property_addr}\n")
                f.write(f"- **Filing Date**: {eviction.get('filing_date', 'Unknown')}\n")
                f.write(f"- **Status**: {eviction.get('status', 'Unknown').capitalize()}\n")
                
                if eviction.get('judgment_date'):
                    f.write(f"- **Judgment Date**: {eviction.get('judgment_date')}\n")
                
                if eviction.get('judgment_amount'):
                    f.write(f"- **Judgment Amount**: ${float(eviction.get('judgment_amount')):.2f}\n")
                
                if eviction.get('back_rent_amount'):
                    f.write(f"- **Back Rent**: ${float(eviction.get('back_rent_amount')):.2f}\n")
                
                if eviction.get('writ_date'):
                    f.write(f"- **Writ Date**: {eviction.get('writ_date')}\n")
                
                if eviction.get('notes'):
                    f.write(f"- **Notes**: {eviction.get('notes')}\n")
...
# Add a route to create some sample data
